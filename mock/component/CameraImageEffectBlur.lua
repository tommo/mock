...
local function buildBlurShader()
	local vsh = [[
	...
	local fsh = [[	
		...
		void main () {
			...
			color += texture2D ( sampler, uvVarying + vec2( 0.0, dv )  );
			color += texture2D ( sampler, uvVarying + vec2( 0.0, -dv ) );
			color += texture2D ( sampler, uvVarying + vec2( du, 0.0 )  );
			color += texture2D ( sampler, uvVarying + vec2( -du, 0.0 ) );
...
function CameraImageEffectBlur:onBuild( prop, layer )
	prop:setShader( assert( buildBlurShader() ) )
end
